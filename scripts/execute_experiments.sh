...
# (set EVAL_HOME if the remote home path differs from this machine's $HOME).
...
pick_next_config() {
    while (( next_idx < ${#CONFIGS[@]} )); do
        ...
        local exp_name done_eval_json
        ((next_idx++))
        if [[ -f "$c" ]]; then
            exp_name=$(basename "$c" .yaml)
            done_eval_json="${LOCAL_EVAL_RESULTS_DIR}/eval_${exp_name}.json"
            if [[ -f "$done_eval_json" ]]; then
                echo "[SKIP] Already has eval result: $done_eval_json"
                continue
            fi
            PICKED_CONFIG=$c
            return 0
...
launch_on_machine() {
    ...
    cmd="uv sync && uv run python -u src/train_with_augmentation.py -c ${config} && mkdir -p ${EVAL_BASE}/gen_ai/eval_results && uv run python -u src/evaluate.py --model ${model} --checkpoint results/${exp_name}/final --output ${EVAL_BASE}/gen_ai/eval_results/eval_${exp_name}.json"
